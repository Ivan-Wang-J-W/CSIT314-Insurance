...
import { resetDataStore } from '../../control/dataStore.js';
...
export default function AdminDashboard() {
  const stats = UserController.stats();
  const overview = ReportController.overview();
  const recentFSAs = FSAController.all()
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    .slice(0, 5);
  const toast = useToast();

  const handleReset = () => {
    if (confirm('Reset all mock data back to initial seed?')) {
      resetDataStore();
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="System-wide statistics and account oversight at a glance."
        actions={
          <Button
            variant="outlined"
            color="inherit"
            ...
            onClick={handleReset}
            sx={{ borderColor: 'divider', color: 'text.secondary', '&:hover': { borderColor: 'text.secondary', bgcolor: 'action.hover' } }}
          >
            Reset Demo Data
          </Button>
        }
      ...
      <Grid container spacing={2.5} sx={{ mb: 4 }}>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            ...
            label="Total Users"
            value={stats.total}
            subtitle={`${stats.active} active · ${stats.suspended} suspended`}
            color="primary.main"
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            icon={<CampaignIcon />}
            label="Active FSAs"
            value={overview.activeFSAs}
            subtitle={`${overview.totalFSAs} total campaigns`}
            color="info.main"
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            icon={<PaymentsIcon />}
            label="Total Raised"
            value={formatCurrency(overview.totalAmountRaised)}
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatCard
            icon={<TrendingUpIcon />}
            label="Avg Donation"
            value={formatCurrency(overview.averageDonation)}
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              <List disablePadding>
                {recentFSAs.map((f, idx) => (
                  <ListItem
                    key={f.id}
                    divider={idx < recentFSAs.length - 1}
