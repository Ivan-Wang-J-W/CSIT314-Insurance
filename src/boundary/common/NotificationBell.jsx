...
export default function NotificationBell() {
  ...
  const refresh = () => setItems(NotificationController.forUser(user.id));
  ...
  const handleClick = (n) => {
    NotificationController.markRead(n.id);
    refresh();
    ...
    <>
      ...
      >
        <Box sx={{ px: 2, py: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {unread > 0 && (
            <Button size="small" onClick={() => { NotificationController.markAllRead(user.id); refresh(); }}>
              Mark all read
            </Button>
          )}
        </Box>
